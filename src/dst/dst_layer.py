...
class Deformable_Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        offset, duration = torch.chunk(offset_duration, 2, -1)
        # offset, duration = offset_duration.chunk(2, dim=-1)
        # offset = offset.tanh_()
        # offset *= self.length
        # offset = offset.clone()
        offset = torch.tanh(offset)  # 如果你确实需要就地操作
        ...
        # duration = duration.sigmoid_() # 如果你确实需要就地操作
        duration = torch.sigmoid(duration) # 如果你确实需要就地操作
        duration = duration * self.length
