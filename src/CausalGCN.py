...
class CausalGCN(torch.nn.Module):
    ...
    def __init__(self, num_features,
        ...
        if args.use_residual:
            self.residual_rc = nn.Sequential(
    ...
    # def forward(self, data, eval_random=True):
        ...
        edge_weight_c = edge_att[:, 0] # 第一列,
        edge_weight_o = edge_att[:, 1] # 第二列
        ...
        # node_att[keypoints, 0] =0
        # node_att[keypoints, 1] =1
        
        
        xc = node_att[:, 0].view(-1, 1) * x # 
        
        xo = node_att[:, 1].view(-1, 1) * x # causal 
        
        xc = F.relu(self.context_convs(self.bnc(xc), edge_index, edge_weight_c))  # 没用
        
        xo = F.relu(self.objects_convs(self.bno(xo), edge_index, edge_weight_o)) # 有用的
        ...
        xc_logis = self.context_readout_layer(xc) # 4 类， 没用 ，无规律
        
        if self.args.use_residual:
            xo_logis = self.objects_readout_layer(xo, self.residual_rc(keypoints_feats))
        xo_logis = self.objects_readout_layer(xo) # 4 类 有用的
        
        xco_logis = self.random_readout_layer(xc, xo, eval_random=eval_random) # 随机混合
...
class CausalGIN(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, keypoints, eval_random=True, train_type="base"):
...
class CausalGAT(torch.nn.Module):
    ...
    def forward(self, x, edge_index, batch, eval_random=True):
        ...
        for i, conv in enumerate(self.convs): # laysers = 3
            x = self.bns_conv[i](x)
