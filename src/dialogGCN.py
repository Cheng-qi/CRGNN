...
class MyRGCNConv(RGCNConv):
    def forward(self, x, edge_index, edge_type = None, edge_norm=None):
        ...
        for i in range(self.num_relations):
            ...
            edge_norm_tmp = edge_norm[edge_type == i]
            if x_l.dtype == torch.long:
                out += self.propagate(tmp, x=weight[i, x_l], size=size)
            else:
                h = self.propagate(tmp, x=x_l, size=size, edge_norm = edge_norm_tmp)
                out = out + (h @ weight[i])

        root = self.root
        if root is not None:
            out += root[x_r] if x_r.dtype == torch.long else x_r @ root * edge_norm[edge_index[0,:] == edge_index[1,:]].reshape(-1,1)
    ...
    def message(self, x_j: torch.Tensor, edge_norm: torch.Tensor) -> torch.Tensor:
        return x_j * edge_norm.reshape(-1,1)