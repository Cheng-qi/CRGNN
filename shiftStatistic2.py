...
class InferenceExperiment(Experiment):
    ...
    def test_epoch_end(self, outputs):
        ...
        shift_labels = torch.cat([x["labels"][x["shiftIndexes"]] for x in outputs]).cpu().numpy()
        shift_outs = torch.cat([x["out"][x["shiftIndexes"]] for x in outputs]).data.cpu().numpy().argmax(-1)

        shift_labels_last = torch.cat([x["labels"][x["shiftIndexes"]-1] for x in outputs]).cpu().numpy()
        shift_outs_last = torch.cat([x["out"][x["shiftIndexes"]-1] for x in outputs]).data.cpu().numpy().argmax(-1)

        acc_num = ((shift_labels == shift_outs) == (shift_labels_last == shift_outs_last)).sum()
        
        error_num = len(shift_labels) - acc_num


        # shift_log = accuracy_score(shift_outs.data.cpu().numpy().argmax(-1), shift_labels.cpu().numpy())
        
        print(f"dia2 | {error_num} | {len(shift_labels)} | {error_num/len(shift_labels)} |")
...
if __name__=="__main__":
    result_dir = f"best_results/iemocap/results/DialogueGCN/fold1"
    best_ckpt = "epoch=11-val_loss=0.80635-val_accuracy=0.80411"
